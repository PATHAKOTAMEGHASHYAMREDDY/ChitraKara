...
// // router.post("/custompaintingrequest", upload.single("image"), async (req, res) => {
// //   try {
// //     const { name, address, artist, customerEmail } = req.body;
// //     const imageUrl = req.file.path;
...
// // Cloudinary Storage Setup for Paintings
...
// // Cloudinary Storage Setup for Custom Painting Requests
// const requestStorage = new CloudinaryStorage({
//   cloudinary: cloudinary,
//   params: {
//     folder: "chitrakara-custom-requests", // Different folder for custom requests
//     allowedFormats: ["jpg", "jpeg", "png"],
//   },
// });
...
//   try {
//     const { name, address, artist, customerEmail } = req.body;
//     const imageUrl = req.file.path; // Image stored in Cloudinary under 'chitrakara-custom-requests'
...
router.post("/order", async (req, res) => {
  try {
    const { customerName, artistName, paintingTitle, price, contact, imageUrl } = req.body;
    const order = new Order({ customerName, artistName, paintingTitle, price, contact, imageUrl });
    await order.save();
    res.status(201).json({ message: "Order placed successfully!" });
  } catch (error) {
    console.error("Order Error:", error);
...
router.post("/custompaintingrequest", requestUpload.single("image"), async (req, res) => {
  try {
    const { name, address, phone, artist, customerEmail } = req.body; // Added phone
    const imageUrl = req.file.path;
    const newRequest = new CustomPaintingRequest({
      name,
      address,
