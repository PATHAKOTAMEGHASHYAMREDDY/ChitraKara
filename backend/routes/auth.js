...
router.post("/chat/message", async (req, res) => {
  try {
    const { sender, receiver, message, requestId } = req.body;
    console.log("Received chat message:", { sender, receiver, message, requestId });
    if (!sender || !receiver || !message || !requestId) {
      return res.status(400).json({ error: "All fields are required" });
    }
...
router.get("/chat/messages/:requestId", async (req, res) => {
  try {
    console.log("Fetching messages for requestId:", req.params.requestId);
...
router.get("/chat/artist/:artistName", async (req, res) => {
  try {
    const chats = await ChatMessage.find({ 
      $or: [{ sender: req.params.artistName }, { receiver: req.params.artistName }]
    })
    .distinct("requestId");
    res.status(200).json(chats);
  } catch (error) {
    console.error("Fetch Artist Chats Error:", error);
