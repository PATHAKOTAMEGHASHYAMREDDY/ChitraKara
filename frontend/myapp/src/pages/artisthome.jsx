...
function Artisthome() {
  ...
  useEffect(() => {
    const fetchCustomRequests = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/customrequests/${username}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/uploadpainting", formData, {
  ...
  const handleAcceptRequest = async (requestId) => {
    try {
      const response = await axios.put(`http://localhost:5000/api/customrequests/${requestId}`, {
        status: "accepted",
      });
